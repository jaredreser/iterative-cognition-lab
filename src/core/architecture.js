(function () {
  ...
  class IterativeCognitionEngine {
    ...
    reset() {
      ...
      this.history = [];
      this.threadTrace = [];
      for (const [name, strength] of Object.entries(this.scenario.seed)) {
        this.foa[name] = strength;
      }
    ...
    step(event = null) {
      ...
      const keepers = this._selectKeepers(slots);
      const nextFoa = Object.fromEntries(keepers);
      ...
      const row = {
        t: this.t,
        foa: Object.keys(this.foa),
        stm: Object.keys(this.stm),
        ssc,
        newlyActive,
        evicted,
        deactivated,
    ...
    _spreadActivation(input, jumpRequested, previousFoa) {
      ...
      for (const [store, storeWeight] of [[this.foa, 1], [this.stm, 0.38]]) {
        for (const [name, strength] of Object.entries(store)) {
          ...
          for (const [target, weight] of Object.entries(this.graph[name] || {})) {
            add(target, strength * weight * storeWeight);
          }
        }
      }
      ...
      for (const [name, strength] of Object.entries(input)) {
        ...
        for (const [target, weight] of Object.entries(this.graph[name] || {})) {
          if (weight > 0) add(target, strength * weight * 0.8);
        }
      }

      if (jumpRequested) {
        for (const name of previousFoa) {
          if (!(name in input)) delete candidates[name];
        }
      }
    ...
    _chooseUpdateSlots(input, forcedSlots) {
      ...
      const highSalience = Object.entries(input).some(([name, value]) => {
        return value > 0.8 && this.nodes[name]?.salience > 0.85;
      });
    ...
    _updateStm(evicted, nextFoa) {
      ...
      for (const [name, strength] of Object.entries(nextFoa)) {
        this.stm[name] = Math.max(this.stm[name] || 0, strength * 0.42);
      }
      this.stm = Object.fromEntries(
        Object.entries(this.stm).sort((a, b) => b[1] - a[1]).slice(0, this.config.stmCapacity)
      );
    }

    _snapshotThread(tag) {
      this.threadTrace.push({
        t: this.t,
        tag,
        foa: Object.keys(this.foa),
      });
